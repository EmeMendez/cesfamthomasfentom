...
<div class="container">
  ...
  {{-- star search --}}
    <div class="row py-3 align-items-center">
      <div class="col-9 pl-2">
        <p class="h4 text-secondary font-weight-bold">Lista de Categorías </p>
      </div>
      <div class="col-3 text-right pr-2">
        <a href="{{route('admin.categories.create')}}" class="text-success"><button class="btn btn-small btn-success">Crear nueva</button></ins></a>        
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-11 pl-0">
        <div class="input-group">
          <input name="name"value="{{old('name',$name)}}" type="text" class="form-control" id="input_search" placeholder="Buscar categoría">
        <a style="text-decoration:none" href="{{route('admin.categories.index')}}">
  ...
  @if(count($categories)>0)
    <div class="row">
        <div class="pt-2 pl-2">
          <p class="text-secondary">Resuldados  @if($name) para <b>{{$name}}</b>  @endif() : [ {{$categories->total()}} ] </p>     
        </div>
